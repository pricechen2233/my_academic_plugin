...
def download_arxiv_(url_pdf,username):
    ...
    download_dir = './gpt_log/'+username+'/arxiv/'
    ...
    x = x.replace('?', '？')\
        ...
        .replace('  ', ' ')\
        .replace('  ', ' ')
    return './gpt_log/'+username+'/arxiv/'+title_str, other_info
...
@CatchException
def 下载arxiv论文并翻译摘要(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port, username):
    ...
    # 提取摘要，下载PDF文档
    try:
        pdf_path, info = download_arxiv_(txt,username)
    except:
        report_execption(chatbot, history, 
            a = f"解析项目: {txt}", 
            b = f"下载pdf文件未成功")
    ...
    import shutil
    # 重置文件的创建时间
    shutil.copyfile(pdf_path, f'./gpt_log/{username}/{os.path.basename(pdf_path)}'); os.remove(pdf_path)
