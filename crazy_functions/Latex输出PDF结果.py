...
def move_project(project_folder, username, arxiv_id=None):
    """ 
    ...
    if arxiv_id is not None:
        ...
        new_workfolder = f'gpt_log/{username}/{gen_time_str()}'
    try: shutil.rmtree(new_workfolder)
    except: pass
    shutil.copytree(src=project_folder, dst=new_workfolder)
    return new_workfolder

def arxiv_download(chatbot, history, txt, username):
    def check_cached_translation_pdf(arxiv_id):
        ...
        if os.path.exists(target_file):
            promote_file_to_downloadzone(target_file,username)
            return target_file
        return False
...
@CatchException
def Latex英文纠错加PDF对比(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port, username):
    ...
    project_folder = move_project(project_folder, username, arxiv_id=None)
    ...
    success = yield from 编译Latex差别(chatbot, history, main_file_original='merge', main_file_modified='merge_proofread', 
                             work_folder_original=project_folder, work_folder_modified=project_folder, work_folder=project_folder, username=username)
...
@CatchException
def Latex翻译中文并重新编译PDF(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port, username):
    # <-------------- information about this plugin ------------->
    chatbot.append([
        "函数插件功能？",
    ...
    # <-------------- clear history and read input ------------->
    history = []
    txt, arxiv_id = yield from arxiv_download(chatbot, history, txt, username)
    if txt.endswith('.pdf'):
    ...
    # <-------------- move latex project away from temp folder ------------->
    project_folder = move_project(project_folder, username, arxiv_id)
