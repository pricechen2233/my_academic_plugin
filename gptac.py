...
def gptac():
    ...
    description =  """基于GitHub开源项目chatgpt_academic[源项目地址🚀](https://github.com/binary-husky/chatgpt_academic)"""
    ...
    with gr.Blocks(title="ChatGPT 学术优化", theme=set_theme, analytics_enabled=False, css=advanced_css) as demo:
        ...
        with gr_L1():
            ...
            with gr_L2(scale=1):
                ...
                with gr.Accordion("基础功能区", open=True) as area_basic_fn:
                    gr.Markdown("注意：以下按钮功能的使用需要先将提示词输入到上方的框中，然后按下按钮触发功能实现，无法从上传文件中读取内容")
                    with gr.Row():
                        for k in functional:
                ...
                with gr.Accordion("函数插件区", open=True) as area_crazy_fn:
                    with gr.Row():
                        gr.Markdown("注意：以下“红颜色”标识的函数插件需从输入区读取路径作为参数，如需自行上传文件，请清除上方输入区内容")
                    with gr.Row():
                        for k in crazy_fns:
                ...
                with gr.Accordion("更换模型 & SysPrompt & 交互界面布局", open=(LAYOUT == "TOP-DOWN")):
                    ...
                    md_dropdown = gr.Dropdown(AVAIL_LLM_MODELS[:2], value=LLM_MODEL, label="更换LLM模型/请求源").style(container=False)
        ...
        # 随变按钮的回调函数注册
        """有严重的bug要修复，原因主要在ArgsGeneralWrapper无法读取cookies上"""
        def route(k, request:gr.Request, *args, **kwargs):
            if k in [r"打开插件列表", r"请先从插件列表中选择"]: return
            #print('args:',args)
            args_ = list(args)
            args_.insert(-1, request)
            #print('args add request',args_)
            yield from ArgsGeneralWrapper(crazy_fns[k]["Function"])(*args_, **kwargs)
