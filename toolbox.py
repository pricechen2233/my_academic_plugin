...
def ArgsGeneralWrapper(f):
    ...
    """
    # 加入了gr.Request置于最后面，这样可以正常在本函数中使用request信息用于检验用户身份，并传递access token到cookie中去
    def decorated(cookies, max_length, llm_model, txt, txt2, top_p, temperature, chatbot, history, system_prompt, plugin_advanced_arg, request:gr.Request, *args):
        txt_passon = txt
        ...
        #print(raw_cookie)
        request_cookies = dict(i.split("=", 1) for i in raw_cookie.split("; "))
        if 'access_token' not in request_cookies.keys():
            raise gr.Error("You are not login, please login!")
        token = request_cookies['access_token'] # 获取token
        # 尝试解码token获取username
        ...
        # 根据数据库用户信息确保用户选择需要的模型
        if llm_model == "gpt-3.5-turbo-16k" and userinfo['grade']%2 == 0 :
            llm_model = "gpt-3.5-turbo"
            raise gr.Error("You are not supported to select gpt-16k model, you must choice gpt-4k model!")
        elif llm_model == "gpt-3.5-turbo-16k" and userinfo['grade']%2 == 1:
            llm_model = "gpt-3.5-turbo-16k"
        elif llm_model == "gpt-3.5-turbo" and userinfo['grade']%2 == 0:
            llm_model = "gpt-3.5-turbo"
        elif llm_model == "gpt-3.5-turbo" and userinfo['grade']%2 == 1:
            llm_model = "gpt-3.5-turbo"
        else:
            raise gr.Error("Unknow error! if you see it, please call administrator!")
        # 引入一个有cookie的chatbot
        cookies.update({
            'top_p':top_p,
...
def CatchException(f):
    ...
    def decorated(txt, top_p, temperature, chatbot, history, systemPromptTxt, WEB_PORT=-1):
        ...
        if username is None: raise gr.Error("You are not login, please login!")
        try:
            yield from f(txt, top_p, temperature, chatbot, history, systemPromptTxt, WEB_PORT, username)
...
def write_results_to_file(history, username, file_name=None):
    """
    ...
    os.makedirs('./gpt_log/'+username, exist_ok=True)
    with open(f'./gpt_log/{username}/{file_name}', 'w', encoding='utf8') as f:
        ...
        for i, content in enumerate(history):
            ...
            f.write('\n\n')
    res = '以上材料已经被写入' + os.path.abspath(f'./gpt_log/{username}/{file_name}')
    print(res)
    return res
...
def promote_file_to_downloadzone(file, username, rename_file=None):
    # 将文件复制一份到下载区
    import shutil
    if rename_file is None: rename_file = f'{gen_time_str()}-{os.path.basename(file)}'
    new_path = os.path.join(f'./gpt_log/{username}', rename_file)
    if os.path.exists(new_path): os.remove(new_path)
    shutil.copyfile(file, new_path)

def on_file_uploaded(files, chatbot, txt, txt2, checkboxes, request:gr.Request):
    """
    当文件被上传时的回调函数
    """
    if len(files) == 0:
    ...
    os.makedirs(f'private_upload/{username}/{time_tag}', exist_ok=True)
    err_msg = ''
    for file in files:
        file_origin_name = os.path.basename(file.orig_name)
        shutil.copy(file.name, f'private_upload/{username}/{time_tag}/{file_origin_name}')
        err_msg += extract_archive(f'private_upload/{username}/{time_tag}/{file_origin_name}',
                                   dest_dir=f'private_upload/{username}/{time_tag}/{file_origin_name}.extract')
    moved_files = [fp for fp in glob.glob('private_upload/'+ username+ '/**/*', recursive=True)]
    if "底部输入区" in checkboxes:
        txt = ""
        txt2 = f'private_upload/{username}/{time_tag}'
    else:
        txt = f'private_upload/{username}/{time_tag}'
        txt2 = ""
    moved_files_str = '\t\n\n'.join(moved_files)
...
def on_report_generated(files, chatbot, request:gr.Request):
    ...
    report_files = find_recent_files('gpt_log/'+username)
...
def zip_result(folder, dest_folder):
    import time
    t = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
    zip_folder(folder, dest_folder, f'{t}-result.zip')
